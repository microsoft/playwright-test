...
it('should work with parameters', async ({ runInlineTest }) => {
  ...
  expect(suites[0].tests[0].variants.length).toBe(3);
  expect(suites[1].file).toContain('b.test.js');
  expect(suites[1].tests[0].variants.length).toBe(3);
  expect(suites[2].file).toContain('c.test.js');
  expect(suites[2].tests[0].variants.length).toBe(3);
  const log = [];
  for (let i = 0; i < 3; ++i) {
    for (const testRun of suites[0].tests[0].variants)
...
it('should emit test annotations', async ({ runInlineTest }) => {
  ...
  expect(result.report.suites[0].tests[0].variants[0].annotations).toEqual([{ type: 'fail', description: 'Fail annotation' }]);
});
...
it('should emit suite annotations', async ({ runInlineTest }) => {
  ...
  expect(result.report.suites[0].suites[0].tests[0].variants[0].annotations).toEqual([{ type: 'fixme', description: 'Fix me!' }]);
});
...
it('should not restart worker', async ({ runInlineTest }) => {
  ...
  expect(result.exitCode).toBe(0);
  expect(result.report.suites[0].tests[0].variants[0].workerId).toBe(0);
  expect(result.report.suites[0].tests[1].variants[0].workerId).toBe(0);
});
